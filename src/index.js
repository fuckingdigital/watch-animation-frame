...
// } 
    const events = [ "onchange", "onappear", "ondisappear", "onmatch", "ondismatch", "onmatchchange"];
    this.addWatch = (options = {}) => {
        
        const watcher = {};
        const id = uuid();

        let changeEvent = false, // for validation check
            matchEvent = false,
    ...
    this.removeNamespace = (namespace) => {
        ...
        for (let i = 0, len = this.namespaces[namespace].length; i < len; i++)
            this.removeWatch(this.namespaces[namespace][i]);
